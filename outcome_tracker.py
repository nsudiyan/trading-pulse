...
def _fetch_price(symbol: str) -> float | None:
    """Получает текущую цену с Bybit V5."""
    try:
        resp = requests.get(
...
def check_and_resolve(silent: bool = False) -> int:
    ...
    for entry in entries:
        ...
        if elapsed_h >= 4 and entry.get("price_4h") is None:
            price_now = _fetch_price(entry["symbol"])
            if price_now is not None:
                ...
                if direction == "ЛОНГ":
                    hit_tp1  = price_now >= tp1 if tp1 else False
                    hit_stop = price_now <= stop if stop else False
                ...
                updated = True
                if not silent:
                    print(f"  [4h] {entry['symbol']:<14} {pct:+.1f}%  {outcome}")
        ...
        if elapsed_h >= 24 and entry.get("price_24h") is None:
            ...
            if price_now is not None:
                ...
                _append_csv(entry)
                if not silent:
                    print(f"  [24h] {entry['symbol']:<14} {pct:+.1f}%  {outcome}")
                time.sleep(0.05)
